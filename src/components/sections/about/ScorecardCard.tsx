...
function RatingDisplay({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-1.5">
      <span className="font-mono text-sm font-bold text-lime-400">{rating}</span>
    ...
    <tr className="group/row border-b border-zinc-800/50 transition-colors hover:bg-zinc-900/50">
      <td className="py-4 pl-4 pr-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-sm text-zinc-200 group-hover/row:text-lime-400">
            {game.name}
          </span>
        </div>
      </td>
      ...
      <td className="py-4 pr-4">
        <span
          title={game.category}
          className="inline-block w-32 truncate rounded border border-emerald-500/30 bg-emerald-950/50 px-2 py-0.5 text-center font-mono text-xs text-emerald-400"
        >
          {game.category}
        </span>
      </td>
...
export function ScorecardCard({
  ...
  return (
    <div
      ...
      className="group relative flex w-full flex-col rounded-lg border border-lime-500/30 bg-zinc-950 p-5 text-left transition-all duration-300 hover:border-lime-400/60 hover:shadow-[0_0_30px_rgba(132,204,22,0.15)] sm:p-6"
    >
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="hidden items-center gap-4 sm:flex">
          <div className="text-right">
            <div className="font-mono text-xl font-bold text-lime-400">{interest.totalPlays}</div>
            <div className="font-mono text-xs text-zinc-500">total plays</div>
          </div>
          <div className="h-8 w-px bg-zinc-800" />
          <div className="text-right">
            <div className="font-mono text-xl font-bold text-emerald-400">{interest.games.length}</div>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-x-6 gap-y-2 rounded border border-zinc-800 bg-zinc-900/50 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-zinc-400">
            Favorite Designer: <span className="text-lime-400">{interest.favoriteDesigner}</span>
          </span>
        </div>
        <div className="hidden h-4 w-px bg-zinc-700 sm:block" />
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs text-zinc-400">
            Top Rated: <span className="text-lime-400">{sortedGames.filter(g => g.rating === 10).length}</span>
          </span>
        </div>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between border-t border-zinc-800 pt-4 sm:hidden">
        ...
        <span className="font-mono text-xs text-zinc-500">
          <span className="font-bold text-lime-400">{interest.totalPlays}</span> plays
        </span>
      </div>
    </div>
  )
